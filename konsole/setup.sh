...
konsole::local::setup() {
  ...
  if [[ ! -d "${HOME}/.fonts/source-code-pro/" ]]; then
    curl --silent --location \
      https://github.com/adobe-fonts/source-code-pro/archive/2.030R-ro/1.050R-it.tar.gz \
        | tar xz --strip-component=1 -C "${HOME}/.fonts/ source-code-pro-2.030R-ro-1.050R-it/OTF"
    mv "${HOME}/.fonts/OTF" "${HOME}/.fonts/source-code-pro"
    fc-cache -f
  fi
}
